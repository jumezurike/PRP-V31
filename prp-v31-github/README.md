...
[![PRP Version](https://img.shields.io/badge/PRP-v3.1%20Final-blue?style=flat-square)](https://b2b.lokdon.com)
[![License](https://img.shields.io/badge/Licence-LokDon%20Open%20Enforcement-green?style=flat-square)](#licence)
...
Every major AI coding assistant — Claude, GPT-4, Gemini, Copilot — operates on a trust model:
*the agent is asked to follow documentation by choice.*

This has been proven insufficient. **Violation 3 (2026-04-17):** an agent began work without reading
session history. The response was not to write better instructions.
The response was to make non-compliance **structurally impossible.**
...
The naming is not metaphorical. It derives from the OKT traversal architecture in the
ECSMID/LFKI framework: each session carries forward the **accumulated history** of all
prior sessions — anomaly signatures, open items, accepted findings — without exposing
the traversal paths that generated them.
...
```bash
git clone https://github.com/lokdon/prp-v31.git
cd prp-v31
chmod +x scripts/nps_gate.sh scripts/nps_work_gate.sh
```
...
- Fill in `docs/templates/SESSION_REPORT_TEMPLATE.txt`
- Save as `tiers/tier3/JU_DEV_YYYYMMDD_HHMM_PR.txt`
- Update `docs/NPS-PROTOCOL.md` with new files
- Run `bash scripts/nps_gate.sh reset`
...
The backup gate, the approval gate, the delivery scan gate, and the NPS startup lock
are structural. Remove them and you have reverted to an honour system that has
...
- [LFKI: OKT-Native Photonic AI Compute Fabric](https://b2b.lokdon.com) — The architecture whose
  Non-Markovian traversal property inspired the NPS gate
